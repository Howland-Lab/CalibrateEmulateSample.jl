...
function postprocess(settings_local, u, v, k, e)
    # u_mean = mean(u_store,dims=2);
    # v_mean = mean(v_store,dims=2);
    # k_mean = mean(k_store,dims=2);
    # e_mean = mean(e_store,dims=2); 
    # ustar_mean = mean(ustar_store);
    # speed = sqrt.( u_mean.^2 .+ v_mean.^2 );
    # dir = atand.(v_mean./u_mean);
   
    # speed_abl = speed[h1:h2];
    # dir_abl = dir[h1:h2];
    # k_abl = k_mean[h1:h2]; k_abl[k_abl .< 1e-20] .= 0;
    # e_abl = e_mean[h1:h2]; e_abl[e_abl .< 1e-20] .= 0;
    
    # Mask data for observable points
    # speed_abl = speed_abl[settings_local.points_mask];
    # dir_abl = dir_abl[settings_local.points_mask];
    # k_abl = k_abl[settings_local.points_mask];
    # e_abl = e_abl[settings_local.points_mask];

    # result = u[settings_local.points_mask];
    result = u[settings_local.points_mask] / u[maximum(settings_local.points_mask)];
    println("Result: ", result);

    return result;

    # Choose output statistics
    # 1. speed, 2. speed dir, 3. speed dir tke, 4. speed dir tke e
    # if settings_local.statistics == 1         
    #     return speed_abl;
    # elseif settings_local.statistics == 2
    #     return vcat(speed_abl, dir_abl);
    # elseif settings_local.statistics == 3
    #     return vcat(speed_abl, dir_abl, k_abl);
    # elseif settings_local.statistics == 4
    #     return vcat(speed_abl, dir_abl, k_abl, e_abl);
    # end
end
