...
include(joinpath(@__DIR__, "gmodel", "gmodel_all.jl"))
...
function main()
    ...
    if avg == "t"
        data_save_directory = homedir * "/run_t_all/"
    elseif avg == "tz"
        data_save_directory = homedir * "/run_tz_all/"
    elseif avg == "txz"
        data_save_directory = homedir * "/run_txz_all/"
    end
    ...
    params_true = [z0_true, zd_true, C_mu_true, C_1_true, C_2_true, C_3_true, sig_k_true, sig_e_true];                             # Vector{Float64}
    param_names = ["z0", "zd", "C_mu", "C_1", "C_2", "C_3", "sig_k", "sig_e"];                                # Vector{String}
    ...
    # priors = PD([prior_z0])
    priors = ParameterDistribution([prior_z0, prior_zd, prior_C_mu, prior_C_1, prior_C_2, prior_C_3, prior_sig_k, prior_sig_e])
