...
include(joinpath(@__DIR__, "gmodel_charles_joint.jl"))
...
function comp2phys_transform(comp_params, prior_lower, prior_upper)
    ...
    for j in 1:size(physical_params,1)
       # Very specific transformations hard-coded for the case where 1:z0, 2:zd
       if j == 1
...
zd_true = 7;    # prescribed displacement height in the wall model for perfect model
params_true = [z0_true, zd_true];                             # Vector{Float64}
param_names = ["z0", "zd"];                                # Vector{String}
...
priors = ParameterDistribution([prior_z0, prior_zd])
...
statistics = 1;
if avg == "t" 
    scratch_dir = "/home/ctrsp-2024/mjchan/joint_charles_data_t";      # Has to be in the PadeOps directory
elseif avg == "txz"
    scratch_dir = "/home/ctrsp-2024/mjchan/joint_charles_data_txz"; 
elseif avg == "tz"
    scratch_dir = "/home/ctrsp-2024/mjchan/joint_charles_data_tz";
end
...
elseif yt_setting == "imperfect"
    ...
    # Example script below.
    if avg == "t"
        yt_csv = joinpath(data_path, "yt_data/imperfect_model_U_t_avg_norm.csv");
    elseif avg == "txz"
        yt_csv = joinpath(data_path, "yt_data/imperfect_model_U_txz_avg_norm.csv");
    elseif avg == "tz"
        yt_csv = joinpath(data_path, "yt_data/imperfect_model_U_tz_avg_norm.csv");	
    end
...
println("Initial ensemble: ", initial_params);
...
println("Physical initial ensemble: ", physical_params);
